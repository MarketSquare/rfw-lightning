...
Float start and stop
    ...
    ${loop} =    Check test and get loop    ${TEST_NAME} 2    0
    Should be IN RANGE loop     ${loop}    4
    Should be FOR iteration    ${loop.body[0]}    \${item}=-1.5
    Should be FOR iteration    ${loop.body[1]}    \${item}=-0.5
    Should be FOR iteration    ${loop.body[2]}    \${item}=0.5
...
Variables in arguments
    ${loop} =    Check test and get loop    ${TEST_NAME}    0
...
Too many arguments
    Check test and failed loop    ${TEST_NAME}    IN RANGE

No arguments
    Check test and failed loop    ${TEST_NAME}    IN RANGE

Non-number arguments
    Check test and failed loop    ${TEST_NAME} 1   IN RANGE
    Check test and failed loop    ${TEST_NAME} 2   IN RANGE
