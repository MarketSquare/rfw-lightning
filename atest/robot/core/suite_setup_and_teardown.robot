...
*** Variables ***
${1_PASS_MSG}     1 test, 1 passed, 0 failed
${1_FAIL_MSG}     1 test, 0 passed, 1 failed
${2_FAIL_MSG}     2 tests, 0 passed, 2 failed
${4_FAIL_MSG}     4 tests, 0 passed, 4 failed
${5_FAIL_MSG}     5 tests, 0 passed, 5 failed
${12_FAIL_MSG}    12 tests, 0 passed, 12 failed
${ALSO}           \n\nAlso teardown of the parent suite failed.
${EXECUTED_FILE}    %{TEMPDIR}/robot-suite-teardown-executed.txt
...
Passing Suite Teardown
    [Setup]    Remove File    ${EXECUTED_FILE}
    Run Tests    ${EMPTY}    core/passing_suite_teardown.robot
    Check Suite Status    ${SUITE}    PASS    ${1_PASS_MSG}
    ...   Test
    File Should Exist    ${EXECUTED_FILE}
    [Teardown]    Remove File    ${EXECUTED_FILE}
...
Failing Higher Level Suite Teardown
    ...
    Check Suite Status    Passing Teardown Dir    FAIL    ${4_FAIL_MSG}
    ...    PTD PTD Passing    PTD PTD Failing    PTD FTD Passing    PTD FTD Failing
    Check Suite Status    Ptd Passing Teardown    FAIL    ${2_FAIL_MSG}
    ...
    Check Suite Status    Ftd Passing Teardown    FAIL    ${2_FAIL_MSG}
...
Failed teardown is noticed when generating only report with Rebot
    ...
    ...    --report report.html --output NONE    ${OUTFILE_COPY}
    Should Be Equal As Integers    ${result.rc}    2
...
*** Keywords ***
Check Suite Status
    [Arguments]    ${suite_or_name}    ${status}    ${message}    @{tests}
    ${is_string} =    Run Keyword And Return Status    Should Be String    ${suite_or_name}
    ${suite} =    Run Keyword If    ${is_string}    Get Test Suite    ${suite_or_name}
    ...    ELSE    Set Variable    ${suite_or_name}
