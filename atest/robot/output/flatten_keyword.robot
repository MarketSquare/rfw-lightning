...
...             --log log.html
${FLAT_TEXT}    _*Content flattened.*_
${FLAT_HTML}    <p><i><b>Content flattened.\\x3c/b>\\x3c/i>\\x3c/p>
${ERROR}        [ ERROR ] Invalid value for option '--flattenkeywords': Expected 'FOR', 'WHILE', 'ITERATION', 'TAG:<pattern>' or 'NAME:<pattern>', got 'invalid'.${USAGE_TIP}\n
...
Pattern match
    Should Be Equal    ${TC.kws[2].doc}    ${FLAT_TEXT}
...
Tag match when keyword has no documentation
    Should Be Equal     ${TC.kws[6].doc}     ${FLAT_TEXT}
...
Flattened in log after execution
    ...
    Should Contain X Times    ${LOG}    ${FLAT_HTML}        6
    Should Contain    ${LOG}    *<p>Doc of keyword 3\\x3c/p>\\n${FLAT_HTML}
    Should Contain    ${LOG}    *${FLAT_HTML}
...
Flatten controls in keyword
    ...
    ${tcf1}=Evaluate    $tc.body[0].body.filter(None,False)
    Length Should Be    ${tcf1}    0
    ${tcf2}=Evaluate    $tc.body[0].body.filter(None,True)
    Length Should Be    ${tcf2}    23
    ${body}=Evaluate    $tc.body[0].body
    Length Should Be    ${body}    23
...
Flatten for loops
    Run Rebot    --flatten For    ${OUTFILE_COPY}
    ${tc} =    Check Test Case    For loop
    Should Be Equal    ${tc.kws[0].type}    FOR
    Should Be Equal    ${tc.kws[0].doc}    ${FLAT_TEXT}
...
Flatten for loop iterations
    Run Rebot    --flatten ForItem    ${OUTFILE_COPY}
    ${tc} =    Check Test Case    For loop
    Should Be Equal    ${tc.kws[0].type}    FOR
    ...
    FOR    ${index}    IN RANGE    10
        ...
        Should Be Equal      ${tc.kws[0].kws[${index}].doc}    ${FLAT_TEXT}
...
Flatten while loops
    Run Rebot    --flatten WHile    ${OUTFILE_COPY}
    ${tc} =    Check Test Case    WHILE loop
    Should Be Equal    ${tc.body[1].type}    WHILE
    Should Be Equal    ${tc.body[1].doc}    ${FLAT_TEXT}
...
Flatten while loop iterations
    Run Rebot    --flatten iteration    ${OUTFILE_COPY}
    ${tc} =    Check Test Case    WHILE loop
    Should Be Equal    ${tc.body[1].type}    WHILE
    ...
    FOR    ${index}    IN RANGE    10
        ...
        Should Be Equal      ${tc.kws[1].kws[${index}].doc}    ${FLAT_TEXT}
...
Invalid usage
    Run Rebot Without Processing Output    ${FLATTEN} --FlattenKeywords invalid   ${OUTFILE_COPY}
...
Run And Rebot Flattened
    ...
    Run Rebot    ${FLATTEN}    ${OUTFILE_COPY}
