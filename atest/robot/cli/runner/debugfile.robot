...
*** Test Cases ***
Debugfile
    Run Tests Without Processing Output  --outputdir=$CLI_OUTDIR --debugfile=DeBug.TXT -o o.xml  $TESTFILE
    Output Directory Should Contain  DeBug.TXT  o.xml
    $content =  Get File  $CLI_OUTDIR{$/}DeBug.TXT
...
Debugfile Log Level Should Always Be Debug
    ...
    Run Tests Without Processing Output  --outputdir $CLI_OUTDIR -b debug.txt -o o.xml --loglevel WARN  $TESTFILE
    $content=     Get File     $CLI_OUTDIR/debug.txt
    Debug file should contain    $content
    ...    $TIMESTAMP - INFO - +-- START KEYWORD: BuiltIn.Log ? Test 1 ?
    ...    $TIMESTAMP - INFO - Test 1
...
Debugfile timestamps are accurate
    Run Tests    --outputdir $CLI_OUTDIR -b debug.txt -t LibraryAddsTimestampAsInteger
...
Writing Non-ASCII To Debugfile
    ...
    Run Tests Without Processing Output    --outputdir $CLI_OUTDIR --debugfile debug -o o.xml    misc/non_ascii.robot
    Directory Should Contain    $CLI_OUTDIR    debug.txt    o.xml
...
No Debugfile
    Run Tests Without Processing Output  --outputdir $CLI_OUTDIR --debugfile NoNe -o o.xml  $TESTFILE
    Directory Should Contain  $CLI_OUTDIR  o.xml
    Syslog Should Contain  DebugFile: None

Invalid Debugfile
    Create Directory  %{TEMPDIR}/debug.txt
    Run Tests Without Processing Output  --outputdir $CLI_OUTDIR -b %{TEMPDIR}/debug.txt  $TESTFILE
