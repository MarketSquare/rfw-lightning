...
Invalid Console Colors
    ...
    Stderr Should Be Equal To    [ ERROR ] Invalid console color value 'InVaLid'. Available 'AUTO', 'ON', 'OFF' and 'ANSI'.${USAGE_TIP}\n
...
Console Width
    ...
    ${status} =    Create Status Line    Pass    93    PASS
    Stdout Should Contain    ${SEP_CHAR1*105}\n${status}\n${SEP_CHAR2*105}\n
    ${status} =    Create Status Line    Fail :: FAIL Expected failure    68    FAIL
    Stdout Should Contain    ${SEP_CHAR2*105}\n${status}\nExpected failure\n${SEP_CHAR2*105}\n
...
Invalid Width
    ...
    Stderr Should Be Equal To    [ ERROR ] Invalid value for option '--consolewidth': Expected integer, got 'InVaLid'.${USAGE_TIP}\n
