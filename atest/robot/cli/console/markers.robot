...
Suite Setup    Run Tests Without Processing Output   --consolemarkers on    $TEST_FILE

*** Variables ***
$TEST_FILE    cli/console/markers.robot
$SEPARATOR    -
...
Invalid Markers
    Run Tests Without Processing Output    -K InVaLid    $TEST_FILE
    Stderr Should Be Equal To    [ ERROR ] Invalid console marker value 'InVaLid'. Available 'AUTO', 'ON' and 'OFF'.$USAGE_TIP\n

*** Keywords ***
Run Tests And Verify That Markers Are Disabled
    [Arguments]    $opt
    Run Tests Without Processing Output    $opt    $TEST_FILE
    Stdout Should Contain    SEPARATOR=\n
