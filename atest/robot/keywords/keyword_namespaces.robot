...
Verify override message
    [Arguments]    ${error_msg}    ${kw}    ${name}    ${standard}    ${custom}=MyLibrary1
    ...    ${std_with_name}=    ${ctm_with_name}=
    ${std_imported_as} =    Set Variable If    "${std_with_name}"    ${SPACE}imported as '${std_with_name}'    ${EMPTY}
    ${ctm_imported_as} =    Set Variable If    "${ctm_with_name}"    ${SPACE}imported as '${ctm_with_name}'    ${EMPTY}
    ${std_long} =    Set Variable If    "${std_with_name}"    ${std_with_name}    ${standard}
    ${ctm_long} =    Set Variable If    "${ctm_with_name}"    ${ctm_with_name}    ${custom}
    ${expected} =    Catenate
    ...    Keyword '${name}' found both from a custom library '${custom}'${ctm_imported_as}
    ...    and a standard library '${standard}'${std_imported_as}. The custom keyword is used.
    ...    To select explicitly, and to get rid of this warning, use either '${ctm_long}.${name}'
    ...    or '${std_long}.${name}'.
    Check Log Message    ${error_msg}    ${expected}    WARN
