...
Leading And Trailing Spaces In Variable Table
    Should Be Equal    $PRE_SPACES    $SP$SPtwo leading spaces
...
Backslash In Variable Table
    Should Be Equal    $BACKSLASH    c:$BStemp$BSnew$BS
    Should Be Equal    $BACKSLASH$BS$BS    c:$BStemp$BSnew${BS*3}
    Should Be Equal    $BACKSLASH$PRE_SPACES    c:$BStemp$BSnew$BS${SP*2}two leading spaces
    Should Be Equal    $BACKSLASH_2    c:$TABemp$NLew
...
Escaping From List Variable In variable Table
    ...
    Should Be Equal    $LIST[1]    c:$BStemp$BS
...
Invalid \\U (32bit) Escape
    ...
    Should Be Equal    \\U00012345    $BSU00012345
